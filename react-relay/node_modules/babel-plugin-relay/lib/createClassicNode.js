      ...
      if (directives.length === 0) {
        ...
        switch (directive.name.value) {
          ...
          case 'relay':
            ...
            require('./invariant')(relayArguments.length === 1 && relayArguments[0].name.value === 'mask', 'BabelPluginRelay: Expected `@relay` directive to only have `mask` argument in ' + 'compat mode, but get %s', relayArguments[0].name.value);
            substitutionName = fragmentName;
...
function createSubstitutionsForFragmentSpreads(t, path, fragments) {
  return Object.keys(fragments).map(function (varName) {
    ...
    if (!fragment.isMasked) {
      require('./invariant')(path.scope.hasBinding(module), 'BabelPluginRelay: Please make sure module %s is imported and not renamed. ' + module);
